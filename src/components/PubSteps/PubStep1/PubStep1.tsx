...
export default function StoreStep1() {
  ...
  const {
    ...
    defaultValues: {
      webImg: [], // Default values are empty on load
    },
  });

  // Charger les données sauvegardées du Zustand lorsque le composant se charge
  useEffect(() => {
    const savedData = formData[1]; // Assurez-vous que formData[1] contient les images
    if (savedData?.webImg) {
      setSelectedImages(savedData.webImg);
      setValue("webImg", savedData.webImg, { shouldValidate: true });
    }
  }, [step, formData, setValue]);

  const onSubmit: SubmitHandler<FormInputs> = (data) => {
    updateStepData(step, { webImg: selectedImages }); // Sauvegarder les images dans le store
    nextStep();
  };
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (files) {
      ...
      if (totalFiles > 3) {
        ...
        setValue("webImg", updatedImages, { shouldValidate: true });
        setError(null);
      }
    }
  };
          ...
          <div>
            ...
            {errors.webImg && (
              <p style={{ color: "red", marginTop: "10px" }}>
                {(errors.webImg.message as string) || ""}
              </p>
            )}
          </div>
